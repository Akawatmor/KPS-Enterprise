...
หลังใช้ Woodpecker CI/CD กระบวนการถูกแปลงเป็น **12-stage logic** ที่ชัดเจนดังนี้ (นับ Stage 0-10 และ Stage 9b)

1. นักพัฒนา `git push` ไปยัง `main`
2. Woodpecker รับ event แล้วรัน pipeline ตาม `.woodpecker/main-push.yml`
3. **Stage 0 (Pre-flight):** สแกน secret (Gitleaks), lint Dockerfile (Hadolint), lint K8s manifest (kube-score), ตรวจ OPA policy (conftest)
4. **Stage 1 (Quality Gates):** ทดสอบ backend (`go test`, coverage ≥30%, gosec, govulncheck) และ frontend (`type-check`, Jest) แบบ parallel
5. **Stage 2 (Integration Test):** รัน integration test กับ Postgres จริง
6. **Stage 3 (Build & Push):** build/push Docker images ของ backend และ frontend แบบ parallel โดยใช้ tag จาก commit SHA
7. **Stage 4 (Security Scan):** sign image ด้วย Cosign, สร้าง SBOM (CycloneDX), scan ด้วย Trivy (block on HIGH/CRITICAL)
8. **Stage 5 (DB Ops):** backup database (pg_dump) และ migration test แบบ parallel
9. **Stage 6 (Canary Deploy):** deploy canary 10%, ตั้งค่า monitoring stack และ AlertmanagerConfig
10. **Stage 7 (Canary Analysis):** เจิน 160 HTTP requests ผ่าน weighted route, ตรวจ Prometheus metrics (5xx, p95 latency)
11. **Stage 8 (Promote/Rollback):** promote เป็น 100% หากผ่าน หรือ auto-rollback และแจ้งทีมทันทีถ้าไม่ผ่าน
12. **Stage 9 (Verification):** smoke test และสร้าง release tag อัตโนมัติ
13. **Stage 9b (Post-deploy Analysis):** k6 load test (20 VUs, 60s) และ DAST ZAP baseline scan
14. **Stage 10 (Notifications):** ส่งอีเมล HTML แจ้งผล Success/Rollback/Failure พร้อม link ไป pipeline, production และ monitoring
...
### 2.4 สิ่งที่ควรโชว์เวลาอธิบายรายงานหรือพรีเซนต์

เพื่อให้รายงานฉบับนี้สอดคล้องกับการนำเสนอจริง ควรเปิด evidence อย่างน้อย 5 จุดต่อไปนี้

| ประเด็นที่อธิบาย | หลักฐานที่ควรเปิด |
|---|---|
| User-facing baseline | หน้า TodoApp Big Calendar และ day panel |
| Runtime baseline | `kubectl get pods,deploy,statefulset -n todoapp` |
| Delivery baseline | `.woodpecker/main-push.yml` และ Woodpecker run ล่าสุด |
| Canary / rollback path | `traefik-routing.yaml`, `canary-analyze`, `promote-stable` หรือ `auto-rollback` |
| Post-deploy evidence | smoke test, monitoring, email notification |

หลักคิดคือ รายงานนี้ไม่ควรเป็นเพียงข้อความอธิบาย แต่ควรชี้ต่อไปยัง evidence ที่ใช้พิสูจน์สิ่งที่รายงานอ้างไว้ได้จริง
...
flowchart LR
  ...
  B --> P0[Stage 0: Pre-flight<br/>gitleaks · hadolint · kube-score · opa]
  P0 --> P1A[quality-backend<br/>go test · gosec · govulncheck]
  P0 --> P1B[quality-frontend<br/>type-check · jest]
  P1A --> P2[integration-test]
  P1B --> P2
  P2 --> P3A[build-push-core]
  P2 --> P3B[build-push-web]
  P3A --> P4[sign-and-scan<br/>cosign · sbom · trivy]
  P3B --> P4
  P4 --> P5A[db-prep]
  P4 --> P5B[migration-test]
  P5A --> P6[apply-cluster-manifests]
  P5B --> P6
  P6 --> P6B[monitoring-bootstrap]
  P6B --> P6C[monitoring-sync]
  P6C --> P6D[canary-deploy 10%%]
  P6D --> P7{canary-analyze<br/>160 reqs · prometheus}
  P7 -->|pass| P8A[promote-stable 100%%]
  P7 -->|fail| P8B[auto-rollback]
  P8A --> P9[smoke-test]
  P9 --> P9B[create-release-tag]
  P9 --> P9C[k6-load-test]
  P9 --> P9D[dast-zap]
  P8A --> P10A[email-success]
  P8B --> P10B[email-rollback]
```
...
4. เขียนไฟล์ pipeline แบบ split ใน `.woodpecker/` โดยยึด `.woodpecker/main-push.yml` เป็น flow หลักของ production
...
3. Stage 0-2 รัน pre-flight checks, `quality-backend`, `quality-frontend` และ integration test
4. `build-push-core` และ `build-push-web` build/push image ใหม่
5. `sign-and-scan`, `db-prep`, `migration-test` รันก่อนถึง canary path
6. `canary-deploy` และ `canary-analyze` ตรวจ path 10% พร้อม metrics
7. `promote-stable` หรือ `auto-rollback` ตัดสินผลก่อน verification ขั้นสุดท้าย
8. `smoke-test`, post-deploy analysis และ email notifications ปิด feedback loop
...
ใส่การเปลี่ยนแปลงใน backend ที่ทำให้ `quality-backend` ไม่ผ่าน หรือใส่การเปลี่ยนแปลงใน frontend ที่ทำให้ `quality-frontend` fail

**ลำดับการตอบสนองของระบบ**

1. Woodpecker trigger pipeline ตามปกติ
2. Stage คุณภาพที่เกี่ยวข้องล้มเหลว (`quality-backend` หรือ `quality-frontend`)
3. Stage ถัดไป เช่น build/push, sign/scan และ deploy path จะไม่ถูกเรียก
...
2. ~~Pipeline ยังไม่มี auto-rollback~~ → **แก้ไขแล้ว:** canary analysis ขั้น Stage 7 จะ trigger `auto-rollback` step อัตโนมัติเมื่อ error rate >5% หรือ p95 >1.5s
3. ~~การ monitor ยังไม่ครบ~~ → **แก้ไขแล้ว:** มี kube-prometheus-stack (Prometheus + Grafana + Alertmanager) พร้อม ServiceMonitor, PrometheusRule, AlertmanagerConfig และ dashboard ถูก deploy ใน pipeline
...
2. ~~frontend quality gate ยังไม่ได้ถูกบังคับ~~ → **แก้ไขแล้ว:** step `quality-frontend` (type-check + Jest) บังคับ parallel กับ `quality-backend` ใน Stage 1 ทุก pipeline run
...
1. ~~pipeline ยังไม่มี vulnerability scanning~~ → **แก้ไขแล้ว:** pipeline มี security layer ครบ 4 ชั้น: (1) Gitleaks secret scan, (2) Hadolint + kube-score + OPA policy, (3) gosec + govulncheck ใน backend, (4) Trivy image scan + Cosign sign + SBOM (CycloneDX) และ DAST ZAP baseline scan หลัง deploy
2. การจัดการ secret ยังพึ่งขั้นตอน manual อยู่พอสมควร (initial provisioning); runtime secrets ถูกจัดการผ่าน K8s Secret และ Woodpecker Secrets
3. เอกสารบางส่วนมีความต่างกันระหว่าง local setup, guide และ runtime setup — ดู `changesmall.md` สำหรับ change log ล่าสุด
...
3. ไฟล์ใน `.woodpecker/` สำหรับ pipeline แต่ละ event โดย `main-push.yml` เป็น flow หลักของ production
...
| Deployment | deploy เร็วขึ้นและลดงาน manual; canary 10%→100% พร้อม auto-rollback | บางขั้นยังต้องเตรียม secret/infra เอง |
| CI/CD | push→ 12-stage logic ครบ: pre-flight, quality gates, sign/scan, canary, monitoring, k6, DAST, release tag, email | initial cluster provisioning ยังเป็น manual |
| Reliability | canary deploy + auto-rollback + Prometheus monitoring + AlertmanagerConfig | PostgreSQL ยังเป็น single replica (ไม่ใช่ HA) |
| Security | DevSecOps ครบ 4 ชั้น: Gitleaks, Hadolint/OPA, gosec/Trivy/Cosign/SBOM, DAST ZAP | image signing ใช้ self-managed key (ไม่ใช่ Sigstore keyless) |
| Collaboration | ใช้ repo เดียวกัน, docs ชัด, pipeline เป็น common visibility, feature flags via ConfigMap | เอกสารบางไฟล์อ้างอิง architecture ยุคต้น Phase 2 ต้องดู changesmall.md |

## 10. ข้อเสนอแนะในการต่อยอดระบบ

### 10.1 ✅ เพิ่ม Frontend Quality Gate ใน Woodpecker (ดำเนินการแล้ว)

step `quality-frontend` ถูกเพิ่มใน Stage 1 (parallel กับ `quality-backend`) รัน `npm run type-check` และ `npm test -- --passWithNoTests --coverage` บังคับก่อน `build-push-web` ทุก pipeline run

### 10.2 ✅ เพิ่ม Security Scan และ Secret Scan (ดำเนินการแล้ว)

ปัจจุบัน pipeline มี security layer ครบ:
- Gitleaks secret scan (Stage 0)
- Hadolint Dockerfile lint + kube-score + conftest OPA policy (Stage 0)
- gosec + govulncheck ใน backend (Stage 1)
- Trivy image scan (HIGH/CRITICAL block) + Cosign image sign + SBOM CycloneDX (Stage 4)
- DAST ZAP baseline scan (Stage 9b, `failure: ignore`)

### 10.3 ✅ Auto-Rollback เมื่อ Canary Analysis Fail (ดำเนินการแล้ว)

`auto-rollback` step ถูก trigger อัตโนมัติเมื่อ canary analysis fail (error rate >5% หรือ p95 >1.5s): weight กลับเป็น 100/0, canary scale down เป็น 0 replica, email แจ้งทีม

### 10.4 ✅ Monitoring Stack (ดำเนินการแล้ว)

kube-prometheus-stack (Prometheus + Grafana + Alertmanager) ถูก deploy ใน `monitoring-bootstrap` step:
- ServiceMonitor สำหรับ todoapp และ Traefik
- PrometheusRule สำหรับ alert rules
- AlertmanagerConfig CR สำหรับ email notification
- Grafana dashboard สำหรับ todoapp metrics
...
| Woodpecker UI | https://woodpecker-kps.akawatmor.com |
| Grafana Monitoring | https://dashboard-kps.akawatmor.com |
...
หากพัฒนาต่อในจุดที่ยังเหลือจริง เช่น signature verification ก่อน promote, restore drill ของ PostgreSQL, log aggregation และ alert tuning ระบบนี้จะขยับจาก production-like baseline ไปสู่ platform ที่ mature ขึ้นได้อย่างชัดเจน โดยไม่ต้องรื้อ baseline หลักของ Phase 2 ใหม่
